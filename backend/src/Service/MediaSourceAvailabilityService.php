...
final class MediaSourceAvailabilityService
{
    ...
    public function isAvailable(string $mediaSource): bool
    {
        if ($mediaSource === 'local') {
            $localBaseUrl = $this->mediaBaseUrls['local'] ?? '';

            return is_string($localBaseUrl) && $localBaseUrl !== '';
        }
    ...
    public function getSourceStatus(string $mediaSource): array
    {
        ...
        if ($mediaSource === 'local') {
            $localBaseUrl = $this->mediaBaseUrls['local'] ?? '';
            $isAvailable = is_string($localBaseUrl) && $localBaseUrl !== '';

            return [
                'source' => 'local',
                'isAvailable' => $isAvailable,
                'isDisabled' => !$isAvailable,
                'status' => $isAvailable ? 'available' : 'unconfigured',
                'message' => $isAvailable ? null : 'Local media serving is disabled; photos are served from remote object storage only.',
            ];
        }
