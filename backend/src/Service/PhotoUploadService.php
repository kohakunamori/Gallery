...
final class PhotoUploadService
{
    ...
    public function upload(array $files): array
    {
        ...
        try {
            ...
            $this->catalog->upsertMany($uploadBatch['catalogItems']);
     ...
     * @param list<UploadedFileInterface> $files
     * @return array{
     *   files:list<array{name:string,path:string,size:int}>,
     *   catalogItems:list<array{path:string,filename:string,takenAt:?string,sortTime:string,width:?int,height:?int,size:int,version:string}>,
    ...
    public function prepareUpload(array $files): array
    {
        ...
        try {
            foreach ($validatedFiles as $validatedFile) {
                ...
                $publishedPath = $this->publishedRelativePath($filename);
                $catalogItem = $this->buildCatalogItem($temporaryPath, $publishedPath, $validatedFile['originalName']);
     ...
     */
    private function buildCatalogItem(string $temporaryPath, string $publishedPath, string $originalName): array
    {
        ...
        $modifiedAt = @filemtime($temporaryPath);
        ...
        return [
            'path' => $publishedPath,
            'filename' => basename($publishedPath),
    ...
    public function streamUploadScriptOutput(?string $uploadDirectory = null): \Generator
    {
        if ($uploadDirectory === null || $uploadDirectory === '') {
            throw new RuntimeException('Upload temporary directory is required.');
        }

        $command = array_merge($this->buildPythonCommandPrefix(), $this->buildUploadScriptArguments($uploadDirectory));
        $descriptorSpec = [
            1 => ['pipe', 'w'],
            2 => ['pipe', 'w'],
        ];
    ...
    private function processEnvironment(?string $uploadDirectory = null): array
    {
        ...
        if ($uploadDirectory !== null) {
            ...
            $environment['UPLOAD_DISCARD_PREPARED_CACHE'] = '1';
        }

        // Prefer the same catalog the PHP API reads/writes so CLI and web stay aligned.
        $catalogPath = $this->catalog->getPath();
        if ($catalogPath !== '') {
            $environment['PHOTO_CATALOG_PATH'] = $catalogPath;
        }
    ...
    {
        // Images go to R2 only; photos-index.json is updated locally via PHOTO_CATALOG_PATH.
        $arguments = [
            $this->scriptPath,
            '--dir',
            $uploadDirectory,
    ...
    /**
     * @param list<array{path:string,filename:string,takenAt:?string,sortTime:string,width:?int,height:?int,size:int,version:string}> $catalogItems
     */
    public function commitCatalogItems(array $catalogItems): void
    {
        $this->catalog->upsertMany($catalogItems);
    }
