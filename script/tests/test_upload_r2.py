...
class PhotoCatalogAndExistingApiTests(unittest.TestCase):
    ...
    def test_list_existing_linux_filenames_reads_filename_and_url_fields(self):
        class FakeResponse:
            def read(self):
                return json.dumps({
                    'items': [
                        {'filename': 'cover.avif'},
                        {'url': 'https://static.cf.nyaneko.cn/gallery/travel/beach%20day.avif?v=1'},
                    ],
                }).encode('utf-8')
        ...
        with patch('urllib.request.urlopen', return_value=FakeResponse()):
            filenames, error = upload_r2.list_existing_linux_filenames()

        self.assertIsNone(error)
        self.assertEqual({'cover.avif', 'beach day.avif'}, filenames)
